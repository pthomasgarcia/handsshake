...
check_isolation() {
    ...
    if [[ "$HOME" == "$REAL_HOME_FOR_GUARD" ]]; then
        echo "FATAL SECURITY ERROR: HOME is not properly mocked!" \
            "Isolation failed." >&2
    ...
    if [[ "$HOME" != *"/handsshake-tests-"* ]]; then
        echo "FATAL SECURITY ERROR: HOME does not appear to be a" \
            "test-specific directory!" >&2
        echo "Current HOME: $HOME" >&2
        return 1
    fi
...
setup_test_env() {
    ...
    # Note: Bash 4.4+ is assumed for ${!prefix@} expansion, which is standard on
    # most linux systems.
    # If not available, we fall back to manual list (but here we assume modern
    # environment).
    ...
    if [[ -f "$BATS_TEST_DIRNAME/../src/core/main.sh" ]]; then
        ...
        HANDSSHAKE_SCRIPT="$(git rev-parse --show-toplevel 2> /dev/null)"
        HANDSSHAKE_SCRIPT="$HANDSSHAKE_SCRIPT/src/core/main.sh"
    fi
