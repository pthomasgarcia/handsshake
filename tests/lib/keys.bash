...
create_test_key() {
    ...
    if [[ "$resolved_path" == "$REAL_HOME_FOR_GUARD"* ]]; then
        echo "FATAL SECURITY ERROR: create_test_key attempted to write to" \
            "real HOME: $resolved_path" >&2
        exit 100
    fi

    # Path depth guard
    if [[ "$resolved_path" == "/etc/"* ]] ||
        [[ "$resolved_path" == "/usr/"* ]] ||
        [[ "$resolved_path" == "/var/"* ]]; then
        echo "FATAL SECURITY ERROR: create_test_key attempted to write to" \
            "system directory: $resolved_path" >&2
        exit 102
    fi
    ...
    if [[ -n "$comment" ]]; then
        ssh-keygen "${key_args[@]}" -f "$key_path" -N "" -C "$comment" \
            > /dev/null 2>&1
    else
...
safe_cp() {
    ...
    if [[ "$resolved_dest" == "$REAL_HOME_FOR_GUARD"* ]]; then
        echo "FATAL SECURITY ERROR: safe_cp attempted to write to" \
            "real HOME: $resolved_dest" >&2
        exit 101
    fi
    cp "$src" "$dest"
}
